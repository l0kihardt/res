...
from ctypes import *
print('LOADING...')
BOUNCE = pkgutil.get_data('test', 'key')
...
exec(len(BOUNCE))
compressed_data = lzma.compress(open('file', 'rb').read())
dump_data = compressed_data[(len(compressed_data) - len(BOUNCE)):]

open('dump4', 'wb').write(dump_data)
