...
key = 0x5E31BC3
def rol(num):
    return ((num >> 31) & 0xffffffff) | ((num << 1) & 0xffffffff)

def decrypt(cipher):
    for i in range(15, -1, -1):
        boxnum = rol(cipher ^ num[i])
        zero = box.index(boxnum & 0xff)
        one = box.index((boxnum >> 8) & 0xff)
        two = box.index((boxnum >> 16) & 0xff)
        three = box.index((boxnum >> 24) & 0xff)
        cipher = zero | (one << 8) | (two << 16) | (three << 24)
    return cipher

fp = open('./flag.locked_65d0360739a01c5737108cc172ea5c8681249c74', 'rb')
fp.read(12)
fp2 = open('./ans', 'wb')
while True:
    tmp = fp.read(4)
    print tmp
    fbytes = int('0x' + tmp[::-1].encode('hex'), 16)
    ans = hex(decrypt(fbytes) ^ key)[2:]
    key = fbytes
    fp2.write(ans.rjust(8,'0').decode('hex')[::-1])
